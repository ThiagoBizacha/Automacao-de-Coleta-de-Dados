    ...
    for index, row in df.iterrows():
        cursor.execute("""
            INSERT INTO public.amazon_nl_final (
                rank, asin, name, title, rating, reviews, currency, value, origin, date, 
                normal_rating, normal_reviews, normal_value, score, value_total
            ) VALUES (%s, %s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
            row['rank'], row['asin'], row['name'], row['title'], row['rating'], row['reviews'],
            row['currency'], row['value'], row['origin'], row['date'], row['normal_rating'], 
